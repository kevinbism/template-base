...
ready(() => {
  ...
  filterElements('.gallery-page-cat__filter', '.gallery-page-item', 'active');
...
function initDario() {
  new Dario('#calendario', {
    ...
    onSelect(dario) {
      ...
      getEl('.qr-m-in').textContent = dario.startMonth;
      getEl('.qr-d-out').textContent = dario.endDate.fullDate;
      getEl('.qr-m-out').textContent = dario.endMonth;
...
function effects() {
  // VARS
  const hamburger = getAll('.hamburger');
  const menu = getEl('.menu');
  const book = getAll('.header-book:not(.header-book--link)');
  const qr = getEl('.qr');
  ...
  const breadcrumbs = getAll('.breadcrumb li:not(:last-child)');
  breadcrumbs.forEach(breadcrumb => {
    breadcrumb.innerHTML += '<i class="fa-thin fa-arrow-right icon"></i>';
  });
}
